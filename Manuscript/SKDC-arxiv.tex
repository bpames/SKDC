...
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
% Enable electronic version.
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
% This package allows if-then-else control structures.
\usepackage{ifthen}
\newboolean{ElectronicVersion}
\setboolean{ElectronicVersion}{true} % CHANGE THIS VALUE AS REQUIRED
%\setboolean{ElectronicVersion}{false}

%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
% Packages to include.
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\usepackage{amsmath,amstext,amsthm} % Lots of math symbols and environments
\usepackage{subfig}
\usepackage{booktabs}
\usepackage{epstopdf}
...
analysis to the case where the clusters are identical in size
...
case) of size $r_1 = \Omega \rbra{ \max \bra{\sqrt{n}, r_2 } } = \Omega \rbra{ \max \bra{\sqrt{n}, n - r_1 } }$.
...
Let $C_{k+1} := V \setminus  (\cup^k_{i=1} C_i)$ and $r_{k+1} := n - \sum_{i=1}^k r_i$ be the size
of  \(C_{k+1}\).
...
\begin{theorem} \label{T: S opt conds}
  ...
  and variances $\{\sigma^2_{ij}\}$.
	...
	be chosen according to  \eqref{eq: mu}, \eqref{eq: lam q},  and \eqref{eq: Xi choice 1}, respectively,
...
\citet[Section~2.8]{lugosi2009}, for more details regarding  the Bernstein inequality.
...
The following bound
on the parametrizing vectors \(\y^{q,s}\) and \(\z^{q,s}\) in the choice of the \((C_q, C_s)\) block of \(\bs\Xi\) defined by~\eqref{eq: Xi choice 1}
...
$
Figure~\ref{fig: rec}(a) illustrates the empirical success rate for each choice of \(\hat r\) and \(p\), as well as the curve \( p =\big((k+1)/(0.65k + 1)\big)(q + \half {\sqrt{n}}),\) where we use the upper bound $\tilde \sigma^2 \le 1/4$ to estimate the constant term in~\eqref{eq:smallest_largest}.
...
\begin{figure}[t]
	...
	\subfloat[{Sparse Noise}]{\includegraphics[width=0.48\textwidth]{sparse_plot.png}}
		...
		The dashed curves indicate the phase transition to perfect recovery predicted by Theorem~\ref{thm: rec}.}
	\label{fig: rec}
\end{figure}
...
\label{app: yz}
% Goal.
We give the full proof of Lemma~\ref{lem: yz bound} in this appendix.
\begin{proof}
...
\begin{align}
    ...
    & \hspace{0.95in}
		...
		\notag \\
		& = O \rbra{ r_s\max\bra{ \sqrt{\tilde \sigma^2 \log n},
...
We next prove Lemma~\ref{lem: S1}.
\begin{proof}
%% Tropp.
...
\begin{align*}
	\| \W_{C_q, C_q} \e - \mu_{qq} r_q \e \| &\le  \|\W_{C_q, C_q}  - \mu_{qq} \e\e^T \| \|\e\| \\
