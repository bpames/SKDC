...
%\bs\lambda_{C_{k+1}} = \frac{1}{2} \rbra{ \alpha - \frac{\mu}{r_{k+1}} } \e.
%\end{equation}
%Note that the formula for \( \bs \lambda_{C_{k+1}}\) is equal to the expectation of an arbitrary \(\bs\lambda_{C_q}\)
%but with \(q = k+1\):
%\[
%\E\sbra{ \bs \lambda_{C_q}} = \frac{1}{2} \rbra{\alpha - \frac{\mu}{r_q}} \e.
%\]
...
\begin{figure}[t]
	\centering
	\subfloat[{Dense noise.}]{\includegraphics[width=0.5\textwidth]{Denseplotq.png} }  
	\subfloat[{Sparse noise.}]{\includegraphics[width=0.5\textwidth]{Sparseplotq.png}} 
...
%The proof for the case when either $q$ or $s$ is equal to $k+1$ is analogous\footnote{For example, if $q = k+1$, then $\left|\lambda_i - \frac{1}{2r_q}(\alpha r_q - \mu)\right| = 0$ (compare to \eqref{eq:temp3}).}. 
...
%$$ \left|\bs{\lambda}_{C_q}^T \bs{e} - \frac{1}{2}\left(\alpha r_q - \mu\right) \right| \leq \frac{1}{2 r_q}r_q^{3/2} = \frac{\sqrt{r_q}}{2}$$
%with probability at least $1-2\exp\left(-2r_q\right)$, and
%$$ \left|\bs{\lambda}_{C_s}^T \bs{e} - \frac{1}{2}\left(\alpha r_s - \mu\right) \right| \leq \frac{1}{2 r_s}r_s^{3/2} = \frac{\sqrt{r_s}}{2}$$
