...
\jmlrheading{}{2018}{}{8/18}{}{Aleksis Pirinen~and Brendan P.W.~Ames}

% Short headings should be running head and authors last names

\ShortHeadings{Exact Clustering of Weighted Graphs via SDP}{Pirinen and Ames}
\firstpageno{1}
...
\begin{theorem} \label{thm: simple}
	...
	with \(k\) disjoint blocks and no outlier nodes, in-cluster edge probability \(p\), and between-cluster edge probability \(q\).
...
The term \emph{outlier nodes} refers to nodes not having a natural assignment to any cluster in the stochastic block model. Each potential edge from each of these nodes to any other node is added independent to the graph with probability $q$, so that each outlier has roughly the same number of neighbours in each cluster block. It is important to note that this edge assignment is performed randomly and not deterministically by an adversary attempting to obscure the cluster structure present in the graph.
...
\begin{algorithm}[t]
  ...
  % \parbox[t]{\dimexpr\linewidth-\algmargin}{\strut #1\strut}}
	...
	\State{\textbf{Data:} Initial iterates $\X^0 = \Y^0 = \Z^0 = \0$, augmented Lagrangian parameter $\rho > 0$, and stopping tolerance $\epsilon > 0$.}

	\State{\textbf{Result:} Approximate solution $(\X^*, \Y^*, \Z^*)$ of \eqref{eq: cluster SDP}.}
	...
	\State
		...
		\]
	\parState{
		Compute estimates of primal and dual objective values
		\[
